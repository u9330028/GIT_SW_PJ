/////////////////////////////////////////////////////////////////////////////
//	DevUtil.cpp : implementation file
/////////////////////////////////////////////////////////////////////////////
/* ──────────────────────────────────────
   ▷ 파일변경이력
  ┌─────┬─────┬────┬────────────────────┐
  ...
  └─────┴─────┴────┴────────────────────┘
    V03-01-01   2005.07.12            기타처리를 위한 함수군
									   * 적용함수 : fnUTL_KillExProgram
   ──────────────────────────────────────*/
/////////////////////////////////////////////////////////////////////////////
#include "stdafx.h"
#include "direct.h"
...
//	외부프로그램 종료처리
/////////////////////////////////////////////////////////////////////////////
int CDevCmn::fnUTL_KillExProgram(LPCTSTR pName)
{
	...
	while (true)
	{
		...
		hwnd = ::FindWindow(NULL, pName);						// Find Windows & Get Her Handle
		if (hwnd == NULL)										// If she is phantom...Nothing to do
			return TRUE;

		GetWindowThreadProcessId(hwnd, &dwProcessId);			// Get ID
		hProcess = OpenProcess(PROCESS_TERMINATE, 0, dwProcessId);	// Open

		if( hProcess )												
		{
			GetExitCodeProcess(hProcess,&dwExitCode);				// 무조건 강제처리(IF처리시 종료치 않는경우 있음)
			TerminateProcess(hProcess,dwExitCode);					// Kill Her
		}
...
//////////////////////////////////////////////////////////////////////////
// for T3ATM 2009.6.17 : 화면교대(Loader<>RearAp)
//////////////////////////////////////////////////////////////////////////
// 화면교대(Loader<>RearAp)
void CDevCmn::fnUTL_SetClerkScreen_T3_Van()
{
//	if (MaxMonitorIndex >= 1)									// 2Monitor이상
	...
	if (RearIsFront)
	{
		...
		m_pDevCmn->fnAPL_PrintJnlInfo(GetSprintf("모드전환 : 거래모드"));
MsgDump(TRACE_CODE_MODE, "Log", __FILE__, __LINE__, "SetClerkScreen_T3_Van:OP_MODE[%s]", "AP");
		uWaitTime = TimerSet(K_10_WAIT);
		while (TRUE)
		{
			Delay_Msg(100);								// Msg & Task
			if (GetShareData("OP_MODE").CompareNoCase("APOK") == 0)
				break;
			if (CheckTimer(uWaitTime))	
				break;
		}
	...
	else
	{
		...
		if(nGetIni == 1)
		{
			fnSPACK_SetEnableControl(CLKEY_OFF, CLKEY_ON, CLKEY_OFF);
		}
		else
		if(nGetIni == 2)
		{
			fnSPACK_SetEnableControl(CLKEY_OFF, CLKEY_OFF, CLKEY_ON);
		}
		else
		if(nGetIni == 3)
		{
			fnSPACK_SetEnableControl(CLKEY_OFF, CLKEY_ON, CLKEY_ON);
		}
		else
			m_pDevCmn->fnSPACK_SetEnableControl(CLKEY_OFF, CLKEY_OFF, CLKEY_OFF);

		fnSPACK_AlarmOnOff(TRUE);
		m_pDevCmn->fnAPL_PrintJnlInfo(GetSprintf("모드전환 : 계원모드"));
MsgDump(TRACE_CODE_MODE, "Log", __FILE__, __LINE__, "SetClerkScreen_T3_Van:OP_MODE[%s]", "OP");
		RearIsFront = 1;
	}
...
ULONG CDevCmn::fnUTL_Asc2ULong(void* lpvoid, int length)
{
	ULONG	ivalue=0;
	int		i;
	...
	for(i=0;i<length;i++)
	{
		if(asc[i]>='0' && asc[i]<='9')
		{
			ivalue = (ivalue*10) + (ULONG)(asc[i]-'0');
		}
	}
	return ivalue;
}

ULONG CDevCmn::fnUTL_ByteLowOrder2Long(BYTE* Src, int inLen)
{
	ULONG m_int = 0;
...
CString CDevCmn::fnUTL_WinExecWaitEndTrace(LPCTSTR lpCmdLind, UINT uCmdShow, DWORD dSleep)
{
	PROCESS_INFORMATION pi; 
    STARTUPINFO si;
    SECURITY_ATTRIBUTES sa;
    HANDLE hReadPipe, hWritePipe;
    char strTBuffer[20480];
	CString strListBuffer("000.000.000.000");
	BOOL				b;
	...
	if (!b) return strListBuffer;

	WaitForSingleObject(pi.hProcess, dSleep);
    CloseHandle(pi.hThread);
	CloseHandle(pi.hProcess);								
    memset(strTBuffer, 0, sizeof(strTBuffer));
    PeekNamedPipe(hReadPipe,strTBuffer,sizeof(strTBuffer), &dwRead,NULL,NULL);

	MsgDump(TRACE_DUMP_MODE, "Log", __FILE__, __LINE__, "%s\n",  strTBuffer);

	strListBuffer.Format("%s", strTBuffer);
	return strListBuffer;
}


int CDevCmn::fnUTL_WinExecWaitEndUtil(LPCTSTR lpCmdLind, UINT uCmdShow, DWORD dSleep)
{
	PROCESS_INFORMATION pi; 
    STARTUPINFO si;
 	BOOL				b;
...
void CDevCmn::fnUTL_PinDataXor(BYTE* src1, BYTE* src2, BYTE* dst,int size)
{
	int i = 0;
	int ivalue = 0;
	while(i< size)
	{
		...
		if(src1[i]>=0x41 && src1[i]<=0x46)
		{
			ivalue = (src1[i]-0x37) ^ ((src2[i]-0x30));
			if(ivalue > 0x09)
				dst[i] = ivalue+0x37;
			else
				dst[i] = ivalue+0x30;
		...
		{
			ivalue = (src1[i]-0x30) ^ ((src2[i]-0x30));
			if(ivalue > 0x09)
				dst[i] = ivalue+0x37;
			else
				dst[i] = ivalue+0x30;
...
void CDevCmn::fnUTL_PinBlockPackXor(BYTE* src1, BYTE* src2, BYTE* dst,int size)
{
	int i = 0;
	int ivalue = 0;
	while(i< size)
...
void CDevCmn::fnUTL_JisPack(BYTE* src, BYTE* dst, int size)
{
	int i = 0;
	int j = 0;
	...
	while(i< size)
	{
		...
		{
			if (src[i] >= 'A')
				dst[j] = (dst[j] & 0x0f) | ((src[i] - 'A' + 10) << 4);
			else
				dst[j] = (dst[j] & 0x0f) | ((src[i] - '0') << 4) ;   
		}
		else
		{
			if (src[i] >= 'A')
				dst[j] = (dst[j] & 0xf0) | ((src[i] - 'A' + 10) & 0x0f);
			else
				dst[j] = (dst[j] & 0xf0) | ((src[i] - '0') & 0x0f);
			j++;
		}
		i++;
	}
...
void CDevCmn::fnUTL_JisPackXor(BYTE* src, BYTE* dst, int size)
{
	int i = 0;
	int j = 0;
	...
	while(i< size)
	{
		...
		{
			if (src[i] >= 'A')
				dst[j] = dst[j] ^ ((src[i] - 'A' + 10) << 4);
			else
				dst[j] = dst[j] ^ ((src[i] - '0') << 4) ;   
		}
		else
		{
			if (src[i] >= 'A')
				dst[j] = dst[j] ^ ((src[i] - 'A' + 10) & 0x0f);
			else
				dst[j] = dst[j] ^ ((src[i] - '0') & 0x0f);
			j++;
		}
		i++;
	}
...
void CDevCmn::fnUTL_UnpackPinData(BYTE* src, BYTE* dst, int size)
{
	int		i, j;
	BYTE	nTempByte;
	...
	while (i < size)
	{
		nTempByte = (src[i] & 0xf0) >> 4;

		if (nTempByte >= 10)
			dst[j] = nTempByte - 10 + 'A';
		else
			dst[j] = nTempByte + '0';
		j++;

		nTempByte = src[i] & 0x0f;
		if (nTempByte >= 10)
			dst[j] = nTempByte - 10 + 'A';
		else
			dst[j] = nTempByte + '0';
...
int CDevCmn::fnUTL_MakeDirectory(char *lpStr)
{
	LONG	lReturn = 0;
	BOOL	bCheck = FALSE;
	char	szDirPath[256];
	...
	iStrLen = strlen(lpStr);
	for (i = 0, k = 0; i < iStrLen; i++)
	{
		if (0x5c == lpStr[i])
		{
			if (i != j)
			{
				...
				if (TRUE == bCheck)
				{
					if (0 == ::CreateDirectory(szDirPath, NULL))
...
///////////////////////////////////////////////////////////////////////////////
int CDevCmn::fnUTL_RegQueryValueExtUtil(LPCTSTR hKeyName, LPCTSTR lpszValueName, DWORD dwType, DWORD dwLen, LPVOID lpszData)
{
	LONG	lResult;
	HKEY	hKeyResult = 0;
...
//		memset(lpszData,NULL,dwLen);							// 초기화 무처리 : 2003.05.18
	...
	// Open Key
	lResult = RegOpenKeyEx(HKEY_CURRENT_USER,hKeyName,0,KEY_ALL_ACCESS,&hKeyResult);
	...
	{
		// Create New Key 
		lResult = RegCreateKeyEx(HKEY_CURRENT_USER,hKeyName,0,"",
...
int CDevCmn::fnUTL_RegSetValueExtUtil(LPCTSTR hKeyName, LPCTSTR lpszValueName, DWORD dwType, DWORD dwLen, LPVOID lpszData)
{
	HKEY	hKeyResult = 0;
	DWORD	dwDisposition = 0;
	LONG	lResult;
...
int	CDevCmn::fnUTL_UserRegGetInt(LPCTSTR hKeyName, LPCTSTR lpszValueName, int InitValue)
{
	fnUTL_RegQueryValueExtUtil(hKeyName, lpszValueName, REG_INT, sizeof(int), &InitValue);
	return InitValue;
}

CString	CDevCmn::fnUTL_UserRegGetStr(LPCTSTR hKeyName, LPCTSTR lpszValueName, LPCTSTR InitValue)
{
	...
	fnUTL_RegQueryValueExtUtil(hKeyName, lpszValueName, REG_STR, sizeof(szData), szData);
	strRet = szData;
	return strRet;
}

int	CDevCmn::fnUTL_UserRegSetInt(LPCTSTR hKeyName, LPCTSTR lpszValueName, int SetValue)
{
	return fnUTL_RegSetValueExtUtil(hKeyName, lpszValueName, REG_INT, sizeof(int), &SetValue);
}

int	CDevCmn::fnUTL_UserRegSetStr(LPCTSTR hKeyName, LPCTSTR lpszValueName, CString SetValue)
{
	char	szData[STRBUFFSIZE] = {0};
	...
	return fnUTL_RegSetValueExtUtil(hKeyName, lpszValueName, REG_STR, SetValue.GetLength()+1, szData);
}

int	CDevCmn::fnUTL_MsgPeekMessageProc(DWORD dTime)
{
	DWORD	Ttime = 0;
	MSG		msg;
	while (TRUE)
...
///////////////////////////////////////////////////////////////////////////////
int CDevCmn::fnUTL_IniGetInt(LPCTSTR szFileName, LPCTSTR szSectionName, LPCTSTR szEntryName, int nDefault)
{
...
///////////////////////////////////////////////////////////////////////////////
CString CDevCmn::fnUTL_IniGetStr(LPCTSTR szFileName, LPCTSTR szSectionName, LPCTSTR szEntryName, LPCTSTR szDefault)
{
	return TRUE;
}

///////////////////////////////////////////////////////////////////////////////
int CDevCmn::fnUTL_IniSetInt(LPCTSTR szFileName, LPCTSTR szSectionName, LPCTSTR szEntryName, int nValue)
{
	...
	if (GetIniName(IniName, szFileName))
	{
		...
		fnUTL_IniFlush(IniName); //Buffer Flush
	}
...
///////////////////////////////////////////////////////////////////////////////
int CDevCmn::fnUTL_TextFileSetStr(LPCTSTR szFileName, LPCTSTR szSectionName, LPCTSTR szEntryName, LPCTSTR szValue)
{
	char	IniName[MAX_PATH];
	int		nRet = NULL;
	HANDLE	m_hFile = NULL;
	...
	char	szStrDummy[2048] = {0};
	char	szStrTime[2048] = {0};
	
	SYSTEMTIME  sTime;
	GetLocalTime(&sTime);
	
	memset(szStrDummy,		0, sizeof(szStrDummy));
	memset(szSection,	    0, sizeof(szSection));
	memset(szStrTime,	    0, sizeof(szStrTime));

	_stprintf(szSection,   _T("[%s]%c%c"),  szSectionName, A_CR, A_LF);
	_stprintf(szStrDummy,  _T("%s=%s%c%c"), szEntryName, szValue, A_CR, A_LF);
	_stprintf(szStrTime,   _T("DateTime=%04d/%02d/%02d %02d:%02d:%02d:%03d%c%c"), sTime.wYear, sTime.wMonth, sTime.wDay, sTime.wHour, sTime.wMinute, sTime.wSecond, sTime.wMilliseconds,A_CR, A_LF);

	if (GetIniName(IniName, szFileName))
	{

		nRet = DeleteFile(IniName);
		for(int i = 0; i < 3; i++)
		{
			if (nRet == TRUE)
				break;
		...
		if ((NULL == m_hFile) || (INVALID_HANDLE_VALUE == m_hFile))
		{
			...
			MsgDump(TRACE_CODE_MODE, "Log", __FILE__, __LINE__, "[UTIL_TextFileSetStr()][★에러] CreateFile Failed = %u", dwRet);
			return FALSE;
		}
		else
		{
			WriteFile(m_hFile, szSection,  strlen(szSection),   &dwWrite, NULL);
			WriteFile(m_hFile, szStrDummy, strlen(szStrDummy),  &dwWrite, NULL);
			WriteFile(m_hFile, szStrTime,  strlen(szStrTime),   &dwWrite, NULL);
			nRet = FlushFileBuffers(m_hFile);
			CloseHandle(m_hFile);
		}
...
///////////////////////////////////////////////////////////////////////////////
int CDevCmn::fnUTL_IniSetStr(LPCTSTR szFileName, LPCTSTR szSectionName, LPCTSTR szEntryName, LPCTSTR szValue)
{
	char	IniName[MAX_PATH];
	int		nRet = NULL;
...
int CDevCmn::fnUTL_IniFlush(LPCTSTR szFileName)
{
	...
	{
		DWORD dwRet = GetLastError();
		MsgDump(TRACE_CODE_MODE, "Log", __FILE__, __LINE__, "[IniFlush()][★에러] CreateFile Failed = %u", dwRet);
		return FALSE;
	}
	else
	{
...
int CDevCmn::fnUTL_ActionJnlPrint(LPCTSTR szSelect, char* fmt, ...)
{
    char		szBuf[STRBUFFSIZE];
	char		szTempFileName[256];
	CFileStatus	TempFileStatus;
	FILE*		TempFp;
	CString		strPrintBuf;
	CString		strSelect(szSelect);
	CString		strFmt(fmt);
	va_list args;
	int 	nBuf = 0;

	if (strSelect.GetLength() < 8)
	{
		strSelect.Format("%s%s", szSelect, SPACE8);
		strSelect = strSelect.Left(8);
	}

	SYSTEMTIME  lTime;
	GetLocalTime(&lTime);
	strPrintBuf.Format("%02d:%02d:%02d:%03d [%s] ", lTime.wHour, lTime.wMinute, lTime.wSecond, lTime.wMilliseconds, strSelect);
	...
	sprintf(szTempFileName, "%s%4.4s\\ACT%2.2s.TXT", _EJR_TXT, &m_pProfile->TRANS.YYYYMMDD[4], &m_pProfile->TRANS.YYYYMMDD[6]);

	// 이전자료 삭제는 fnAPL_InitializeDS()
	TempFp = fopen(szTempFileName, "a");
	if (TempFp == NULL)
	{
		MsgDump(TRACE_CODE_MODE, "Log", __FILE__, __LINE__, "fnAPL_ActionJnlPrint():File Open Fail! [%s]", szTempFileName);
		return T_OK;
	}

	fwrite((LPCTSTR)strPrintBuf, strPrintBuf.GetLength(), 1, TempFp);
	fclose(TempFp);
...
int CDevCmn::fnUTL_RunCheckProcess(CString szAtmName)
{
	szAtmName.MakeUpper();
	...
	if ( (int)hSnapshot != -1 )
	{
		...
		if ( Process32First ( hSnapshot, &pe32 ) ); 
		{
			do
			{
				...
				if( ( strProcessName.Find(szAtmName,0) != -1 ) )
				{
					return TRUE;
				}
...
int CDevCmn::fnUTL_RunCheckProcess(LPCTSTR szAtmName)
{
	...
	for( i = 0; i < dwProcessCount; i++ )
	{
		...
		if( hProcess )
		{
									...
									&dwNeeded) )
			{
				...
				{
					CloseHandle(hProcess);						// V06-27-00-#08
					break;
				}
			}
			// V06-27-00-#08
...
int CDevCmn::fnUTL_FTP_Download_File(int bF)
{
	...
	CString szUsrPwd("");
	CString szLocalPath("");
	CString szRemotePath("");
	CString strGetVerion("");
	CString strGetLocalFilePath("");
	CString strGetRemoteFilePath("");

#ifdef CERTI_TEST_KEY000
	szSvrIp		= "172.1.12.109";//IniGetStr(_ATMS_FTP_INI, "FTP_SVR","IP000", "172.1.12.109");	
#else														//  
	szSvrIp		= "211.55.3.251";//IniGetStr(_ATMS_FTP_INI, "FTP_SVR","IP000", "211.55.3.251");	
#endif	

	
//	szSvrIp		= "172.1.12.109";
	szSvrName	= "ECASH";        //IniGetStr(_ATMS_FTP_INI, "FTP_SVR","SNAME", "ECASH");	
	szUsrName	= "autm_user";    //IniGetStr(_ATMS_FTP_INI, "FTP_SVR","UNAME", "autm_user");		
	szUsrPwd	= "autm_userpass";//IniGetStr(_ATMS_FTP_INI, "FTP_SVR","UPSWD", "autm_userpass");	
	//C:\\Update\\VER.ini ([VERSION] VER_FILE = V0823P0001__1234512345.zip)
	szLocalPath = "C:\\Update";   //IniGetStr(_ATMS_FTP_INI, "FTP_SVR","LPATH","C:\\Update");
    
	strGetLocalFilePath = szLocalPath + "\\" + "VERLIST.INI";
	szRemotePath = "/autm_down/HyoSung"; //#CS0001
	strGetRemoteFilePath = szRemotePath + "/DOWNLOAD/" + "VERLIST.ini";

	BOOL bRepeat = FALSE;
	int  nDnStep = 0;
	MsgDump(TRACE_CODE_MODE, "log", __FILE__, __LINE__, "fnUTL_FTP_Download_File:[FTP_SVR_IP:%s, FTP_SVR_NAME:%s, FTP_USR_NAME:%s, FTP_USR_PWD:%s, FTP_LOCAL_PATH:%s, FTP_REMOTE_PATH:%s]",szSvrIp, szSvrName, szUsrName, szUsrPwd, strGetLocalFilePath, strGetRemoteFilePath);

	CRemoteFile rf(szSvrName, szSvrIp, szUsrName, szUsrPwd);
	...
	do
	{
		nErr = rf.GetFile(strGetRemoteFilePath, strGetLocalFilePath, dwOptions);
		MsgDump(TRACE_CODE_MODE, "log", __FILE__, __LINE__, "fnUTL_FTP_Download_File:[%d]",nErr);
		switch (nErr)
		{
		case RMERR_SUCCESS:
			nDnStep = 1;
		...
		default:
			{
				...
				if (szLocalPath.IsEmpty())
					sMessage.Format("Sorry, the requested upload to '%s' could not be completed \nfor the following reason:\n\n%s",
									szSvrIp, rf.GetLastError());
				else
					sMessage.Format("Sorry, the download of '%s' to '%s' \ncould not be completed for the following reason:\n\n%s",
									szRemotePath, szSvrIp, rf.GetLastError());

				MsgDump(TRACE_CODE_MODE, "log", __FILE__, __LINE__, "fnUTL_FTP_Download_File:[%s]",sMessage);
			}
			nDnStep = 0;
			break;
		}
	...
	if(nDnStep == 0) return T_ERROR;

	MsgDump(TRACE_CODE_MODE, "log", __FILE__, __LINE__, "fnUTL_FTP_Download_File:Step1 [%d]",nDnStep);


	strGetVerion = IniGetStr(strGetLocalFilePath, "VERSION", "VER_FILE", "V0000000__00000000.zip");
	strGetLocalFilePath.Empty();
	strGetRemoteFilePath.Empty();
	strGetLocalFilePath = szLocalPath + "\\" + strGetVerion;
	strGetRemoteFilePath = szRemotePath + "/DOWNLOAD/" + strGetVerion;

	MsgDump(TRACE_CODE_MODE, "log", __FILE__, __LINE__, "fnUTL_FTP_Download_File:Step1 [Localpath = %s remote path = %s]",strGetLocalFilePath, strGetRemoteFilePath);
	...
	if(nDnStep == 0)
	{
		DeleteFiles(szLocalPath);
		return T_ERROR;
	}
	MsgDump(TRACE_CODE_MODE, "log", __FILE__, __LINE__, "fnUTL_FTP_Download_File:Step2 [%d]",nDnStep);
	...
	MsgDump(TRACE_CODE_MODE, "log", __FILE__, __LINE__, "fnUTL_FTP_Download_File Complete!!");
...
void CDevCmn::fnUTL_HangulCodeConvert(int nFlag, char* strSrc, char* strDest, int nSize)	
{
  char  szSrc[256];
  int  nIndex=0;
  ...
  /*strcpy(szSrc, (LPCTSTR)strSrc);*/
   strcpy(szSrc, (char*)strSrc);
...
void CDevCmn::TG2KS(unsigned char *c1, unsigned char *c2)	
{
...
void CDevCmn::KS2TG(unsigned char *c1, unsigned char *c2)	
{
	unsigned int c ;
...
unsigned int CDevCmn::Binary(unsigned int val, unsigned int range)		
{
